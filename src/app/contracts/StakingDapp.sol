...
function deposit(uint _pid, uint _amount) public nonReentrant{
    ...
    _createNotification(_pid, pending, msg.sender, "Claimed");
    }
...
_createNotification(_pid, _amount, msg.sender, "Deposited");
    
    }
function withdraw(uint _pid, uint _amount) public nonReentrant {
    PoolInfo storage pool = poolInfo[_pid];
    UserInfo storage user = userInfo[_pid][msg.sender];
    require (user.amount >= _amount, "Withdrawal amount exceeds balance");
...
user.lastRewardAt = block.timestamp;
_createNotification(_pid, _amount, msg.sender, "Withdrawn");
...
function _calcPendingReward(UserInfo storage user, uint _pid) internal view returns (uint) {
    ...
    // Calculate the number of days passed (in minutes for your use case)
    uint daysPassed = (block.timestamp - user.lastRewardAt) / 60;

    // Cap daysPassed at pool.lockDays
    if (daysPassed > pool.lockDays) {
        daysPassed = pool.lockDays;
    }

    // Calculate the pending reward
    uint pendingReward = (user.amount * daysPassed * pool.apy) / (365 * 100);
...
function sweep(address token ,uint256 amount ) external onlyOwner{
    ...
    require(amount <= token_balance, "Insufficient token balance");

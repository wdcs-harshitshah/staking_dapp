...
interface ERC20 {
    ...
    function transferForm (address spender, address recipient, uint256 amount) external returns(bool);
...
contract TokenICO {
    ...
    modifier onlyOwner(){
        require(msg.sender == owner, "Only contract owner can performthis action");
        _;
    }
constructor(){
    owner = msg.sender;
...
function getTokenDetails() public view returns (string memory name, string memory symbol,uint balance, uint256 supply, uint256 tokenPrice, address tokenAddr ){
ERC20 token = ERC20(tokenAddress);
return (
    token.name(),
    token.symbol(),
